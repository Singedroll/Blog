...
export const BlogHero = () => {
  ...
  if (error) {
    return <p>Sorry! Internal server occured please reload again.</p>;
  }

  const handlePrev = () => {
    // Go to the previous article, looping back to the last one if at the first
  ...
  const handleNext = () => {
    // Go to the next article, looping back to the first one if at the last
    setCurrentIndex((prevIndex) =>
